  ...
  return (
    <PageShell>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <ScoreCard label="Quiz svolti" value={loading ? "..." : stats?.completedQuizzes ?? "—"} />
        <ScoreCard label="Domande risposte" value={loading ? "..." : stats?.answeredQuestions ?? "—"} />
        <ScoreCard label="Percentuale corrette" value={loading ? "..." : stats ? `${stats.accuracy}%` : "—"} />
        <ScoreCard label="Tempo medio" value={loading ? "..." : stats ? `${stats.averageDuration}s` : "—"} />
      </div>
      <div className="mt-5">
        {error ? (
          ...
          <div className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow-card)]">
            <p className="text-sm font-semibold text-[var(--muted)]">Ultimo quiz</p>
            <p className="mt-2 text-2xl font-bold">{stats.latestQuiz.score}%</p>
            <p className="mt-1 text-sm text-[var(--muted)]">{stats.latestQuiz.totalQuestions} domande completate</p>
          </div>
        ) : !loading ? (
